...
以下はすべて独立したアプリケーションとして動作する。

- 単語道場
- 単語道場-Reverse
- Study DB

単語道場と単語道場-Reverseは、アプリケーションとしては独立しているが、同じGoogleスプレッドシートのデータを共有して使用する。
...
「単語道場」と「単語道場-Reverse」は、同じGoogleスプレッドシートを共有して使用する。
...
単語道場と単語道場-Reverseは、同じ「単語リスト」の単語データを使用する。

Reverse版専用の別の単語リストは作成しない。

Study DBも、同じ共通スプレッドシートの「単語リスト」と「URL台帳」をGAS経由で利用する。

アプリケーションを共有するのではなく、データを共有する構成とする。
...
imageパラメータがある場合は、Google Drive画像の表示URLを返す。
...
「単語リスト」の課Noと「URL台帳」の課Noを照合し、各カードに以下のURLを付加する。

jpThUrl
thJpUrl
studyDbUrl

これにより、単語道場・単語道場-Reverse・Study DBが、同じURL台帳を基準として各課のURLを取得できる。
...
「単語道場」と「単語道場-Reverse」は、同じスプレッドシートの「単語リスト」を共有して使用する。

Study DBも同じ共通スプレッドシートの「単語リスト」をGAS経由で使用する。
...
意味画像は単語道場・単語道場-Reverseで共有する。

日本語画像も単語道場・単語道場-Reverseで共有する。
...
3つのアプリケーションは、それぞれ独立したアプリケーションとして維持する。
...
Study DBでlessonパラメータにより課Noを判定し、URL台帳の同じ課Noの行からURLを取得する。
...
単語道場・単語道場-Reverseの設定モーダルには「戻る」ボタンを配置する。

「戻る」を選択すると、現在の課Noに対応するURL台帳D列のStudy DB URLへ戻る。
...
のように、通常版・Reverse版のどちらからでも同じ課のStudy DBへ戻る。

Study DB URLはアプリケーション側に固定記述せず、URL台帳D列を基準として使用する。
...
単語データの追加・編集は、共通スプレッドシートの「単語リスト」を基準として行う。

単語道場と単語道場-Reverseの両方が同じ単語データを使用する。

Reverse版専用の単語データは作成しない。

Study DBも共通の単語データとURL台帳を利用するが、単語道場・Reverseとは独立したアプリケーションとして維持する。

URL台帳を変更することで、各課からジャンプする先のURLを管理できる。

単語道場・単語道場-Reverseのどちらからでも、現在の課Noに対応するURL台帳D列のStudy DB URLへ戻ることができる。

通常版とReverse版は同じGAS・同じスプレッドシートを共有して使用する。

一方のアプリケーションコードを変更しても、もう一方のアプリケーションコードが直接変更されることはない。

共通するのはデータ基盤であり、アプリケーション自体は独立している。
